...
const sound = {
  ...
  style: 'Ambient',
  price: '79,99',
  waveType: '/assets/beats/Cristals.mp3',
  imageUrl: '/assets/img/cristalsbg.jpg',
};

const Player = () => {
  const [play, setPlay] = useState(false);
  const cristalsRef = useRef<HTMLAudioElement>(null);
  const [duration, setDuration] = useState<number | null>(null);
  const [currentTime, setCurrentTime] = useState<number>(0);
  const MAX = 20;

  useEffect(() => {
    const audio = cristalsRef.current;
  ...
  // useEffect(() => {
  //   if (cristalsRef.current) {
  //     cristalsRef.current.play();
  //     setPlay(true);
  //   }
  // }, [sound.waveType]);
  ...
  const toggleAudio = () => {
    if (play) {
      cristalsRef.current?.pause();
      setPlay(false);
    } else {
      void cristalsRef.current?.play();
      setPlay(true);
    }
  };
  ...
  const handleVolume = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    if (cristalsRef.current) {
      cristalsRef.current.volume = volume;
    }
  };
  ...
  const handleSeek = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const currentAudio = cristalsRef.current;
...
<main>   
     <div className="bg-accent flex h-fit max-w-fit flex-col rounded-lg border-2 border-cyan-700 pb-4 text-center shadow">
        <div className="relative flex flex-col space-y-0">
          ...
          <dl className="mt-1 flex flex-col p-4 ">
            <dd className="text-lg text-white">{sound.style}</dd>
          </dl>
          <dl className="mt-1 flex flex-col p-4 ">
            <dd className="text-lg text-white">{sound.price}</dd>
          </dl>
      ...
      <audio ref={cristalsRef} src={sound.waveType} />
      </main>
  );
};
